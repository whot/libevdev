 ...
 * ==============================
 * Event names and input property names are affected by the build-time
 * environment. The list of event names is compiled from linux/input.h at
 * build-time and thus any events not defined on the build environment will
 * not resolve. Specifically, libevdev_event_code_get_name(type, code) for an
 * undefined type or code will always return NULL. Likewise,
 * libevdev_property_get_name() will return NULL for properties undefined on
 * the build system.
 *
 * Input properties
 * ================
