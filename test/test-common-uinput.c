...
static char* get_devnode(struct uinput_device *d)
{
	const char *syspath = libevdev_uinput_get_syspath(d->uidev);
	DIR *dir;
	struct dirent *dp;
	struct udev *udev;
...
uinput_device_event(const struct uinput_device *dev, unsigned int type, unsigned int code, int value)
{
	...
	rc = write(dev->dev_fd, &ev, sizeof(ev));
